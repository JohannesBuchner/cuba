...
:Evaluate: System`MinPoints::usage = "MinPoints is an option of Cuhre.
	It specifies the minimum number of points to sample."

:Evaluate: System`Final::usage = "Final is an option of Cuhre.
...
:Evaluate: System`Regions::usage = "Regions is an option of Cuhre.
...
:Evaluate: System`Region::usage =
	"Region[ll, ur, res] conveys information on a subregion.
...
:Begin:
:Function: Cuhre
:Pattern: MLCuhre[ndim_, ncomp_, epsrel_, epsabs_, flags_, mineval_, maxeval_, key_]
:Arguments: {ndim, ncomp, epsrel, epsabs, flags, mineval, maxeval, key}
:ArgumentTypes: {Integer, Integer, Real, Real, Integer, Integer, Integer, Integer}
:ReturnType: Manual
:End:
...
:Evaluate: Cuhre[f_, v:{_, _, _}.., opt___Rule] :=
	...
	rel, abs, minevalk, maxeval, key, verbose, final, regions, compiled},
...
static void DoSample(cnumber n, real *x, real *f)
{
  ...
  MLPutFunction(stdlink, "EvaluatePacket", 1);
  MLPutFunction(stdlink, "Cuhre`Private`sample", 1);
